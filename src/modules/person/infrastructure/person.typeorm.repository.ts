...
const getTypeOrmRepository = () =>
    dataSource.getRepository<PersonDao>(PersonDao).extend({
        findOne(props: {
...
export function getTypeOrmPersonRepository(manager: AbstractTransactionManager) {
    const typeOrmPersonRepository = getTypeOrmRepository();
